...
const authSecretKey = async (
    ...
    next: NextFunction) => {
    try {
        const authHeader = req.headers.secret_key

        if (!authHeader) throw new CustomError(httpCode.unauthorized, "error", "Unauthorized")

        if (authHeader !== getConfig("API_KEY")) throw new CustomError(httpCode.unauthorized, "error", "Token Not Match")
